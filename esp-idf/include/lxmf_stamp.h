 ...
 * big-endian 256-bit integer, is <= 2^(256 - cost). The workblock is
 * 768 KB of HKDF-SHA256 expansion of the message_id over
 * WORKBLOCK_EXPAND_ROUNDS rounds — a protocol constant (both ends must
 * derive the identical block), NOT a tunable. Its rounds are derived
 * independently and consumed strictly in order, so it is streamed
 * through the hash 256 bytes at a time and never held in memory.
 *
 ...
 * means "no stamp required" and always returns true. The workblock is
 * streamed through the hash round by round — no allocation. */
